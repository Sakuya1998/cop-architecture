# COP Observability Stack Content Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 将 `COP-INFRA-004` 从初始化骨架补充为产品中立、可验证的 Observability capability、Telemetry data path、authority、failure、security、lifecycle 与 recovery 架构文档。

**Architecture:** 只修改 `docs/infrastructure/observability-stack.md`，采用 `Capability + Data Path + Evidence Contract`。文档定义 Collector/Adapter、Telemetry Pipeline、Telemetry Backend、Query Runtime、Platform Health Evidence 五类逻辑角色，以及 Managed-Environment Observability、Platform Observability、受控查询、backpressure/loss、security/lifecycle、Resume/Replay/Reconcile/Never infer 和三个既有 Evolution Profile；不固定产品、协议、schema、物理 topology 或数值参数。
...
- Modify: `docs/infrastructure/observability-stack.md`
  - 唯一实施目标；拥有 Collector/Adapter、Telemetry pipeline、Backend/query runtime、Platform Observability、failure/loss 与 recovery 语义。
- Reference only: `docs/superpowers/specs/2026-08-12-observability-stack-content-design.md`
  - 已批准设计；实施不得扩大其范围。
...
- Reference only: `docs/api/event-contracts.md`
  - 提供逻辑责任、Managed Connector/Collector、Contract version、at-least-once、duplicate、out-of-order 与 replay 语义。
- Reference only: `docs/domains/resource-metadata-domain.md`
- Reference only: `docs/domains/observability-domain.md`
- Reference only: `docs/domains/alerting-domain.md`
  - 分别拥有 Resource Identity、Telemetry Source/Query Intent/Evaluation Input 与 Alert outcome/lifecycle。
...
- Reference only: `docs/infrastructure/network-and-ingress.md`
  - 提供 Telemetry Backend authority、Collector placement、raw Telemetry lifecycle、Controlled Egress、Managed Session 和 Evolution Profile 不变量。
- Reference only: `docs/security/security-architecture.md`
- Reference only: `docs/security/audit-and-compliance.md`
  - 分别拥有 security control 与 Audit/Compliance 语义；本任务不修改任何参考文件。

### Task 1: Define the Observability Stack Architecture document
...
$path = 'docs/infrastructure/observability-stack.md'
...
foreach ($required in @(
  'Capability + Data Path + Evidence Contract',
  'Collector / Adapter',
  'Telemetry Pipeline',
  'Telemetry Backend',
  'Query Runtime',
  'Platform Health Evidence',
  'Managed-Environment Observability',
...
Expected: FAIL with `Front matter mismatch` because the current stub is version `0.1.0`, names products, and lacks the approved capability/evidence model.

- [ ] **Step 2: Replace the stub with the approved target content**

Replace `docs/infrastructure/observability-stack.md` with exactly:
...
foreach ($required in @(
  'Capability + Data Path + Evidence Contract',
  'Managed-Environment Observability', 'Platform Observability',
  'Collector / Adapter', 'Telemetry Pipeline', 'Telemetry Backend', 'Query Runtime', 'Platform Health Evidence',
  'outbound authenticated session', 'Controlled Egress',
  'at-least-once', 'bounded', 'backpressure', 'duplicate', 'delay', 'out-of-order', 'drop', 'sampling', 'throttling',
  'Query Intent', 'immutable Evaluation Input', 'result integrity reference',
  'source silence', 'collection gap', 'blind spot',
  'Secret/KMS Reference', '跨 Tenant 默认拒绝',
  'Resume', 'Replay', 'Reconcile', 'Never infer',
  'MVP Self-hosted Baseline', 'Resilient Self-hosted', 'SaaS-ready Isolation'
)) {
  if (-not $content.Contains($required)) { throw "Missing requirement: $required" }
}
foreach ($product in @('OTel','OpenTelemetry','VictoriaMetrics','Prometheus','Grafana','Loki','Tempo','Jaeger','Elasticsearch')) {
  if ($content.Contains($product)) { throw "Product binding found: $product" }
}
...
if ([regex]::IsMatch($diagram, '(?m)^\s*(QUERY|BACKEND|PIPELINE|COLLECTOR|HEALTH)\s+(?:-->|-\.->).*?\bALERT\s*$')) { throw 'Alerting authority bypass found' }
if ([regex]::IsMatch($diagram, '(?m)^\s*(OBS|ALERT|AUDIT|HEALTH)\s+(?:-->|-\.->).*?\bBACKEND\s*$') -and -not $diagram.Contains('QUERY -->|"authorized query"| BACKEND')) { throw 'Telemetry Backend access bypass found' }
$capabilityTable = [regex]::Match($content, '(?ms)^\| Capability role \| Responsibility \| Must not own or infer \| Recovery contract \|\n(.*?)\n\n')
...
$stageTable = [regex]::Match($content, '(?ms)^\| Stage \| Explicit states \|\n(.*?)\n\n')
if (-not $stageTable.Success) { throw 'Stage table missing' }
$stageRows = @($stageTable.Groups[1].Value -split [char]10)
if ($stageRows.Count -ne 6) { throw "Expected separator and 5 stage rows, found $($stageRows.Count)" }
$profileTable = [regex]::Match($content, '(?ms)^\| Evolution profile \| Observability capability and isolation \| Invariants and entry evidence \|\n(.*?)\n\n')
...
foreach ($row in $capabilityRows) { if (([regex]::Matches($row, '\|')).Count -ne 5) { throw "Capability table column mismatch: $row" } }
foreach ($row in @($stageRows + $profileRows)) { if (([regex]::Matches($row, '\|')).Count -ne 3 -and ([regex]::Matches($row, '\|')).Count -ne 4) { throw "Table column mismatch: $row" } }
...
if ($links.Count -ne 13) { throw "Expected 13 reference links, found $($links.Count)" }
...
'PASS: Observability Stack structure and semantics'
```

Expected output: `PASS: Observability Stack structure and semantics`.

- [ ] **Step 4: Perform the manual authority, failure and recovery review**

Read the target source or rendered Markdown and confirm:

- raw Metrics/Logs/Traces and query-engine internals remain Telemetry Backend external authority even when operator-managed or physically co-located;
- Observability owns Source, Binding, Query Intent, completeness and immutable Evaluation Input; Alerting owns evaluation outcome and Alert lifecycle; Resource Metadata owns Resource Identity/Context;
- Collector/Adapter, Pipeline, Query Runtime and Platform Health Evidence remain infrastructure roles and cannot create domain facts, authorization decisions, Audit Evidence or completed state;
- Managed-Environment and Platform Observability can share physical capability while Source, Tenant, classification, pipeline/query scope, access, failure and recovery remain independently verifiable;
- Managed Collector is outbound-only, session identity is separate from task authorization, and source discovery cannot expand scope;
- Controlled Adapter access uses Controlled Egress and Secret/KMS Reference; network reachability or authentication alone does not authorize query;
- transforms are limited to validation, normalization, filtering, redaction, minimization, routing, batching, compression, governed sampling and versioned enrichment reference;
- delivery is at-least-once with bounded resource use and explicit duplicate, delay, out-of-order, replay, drop, sampling, throttling, rejection and unknown outcome evidence;
- empty results do not hide source silence, collection gap, pipeline drop, Backend failure, authorization filtering or query incompleteness;
- Query Runtime returns `as_of`, time coverage, freshness, completeness, failure evidence and provenance, while only Observability forms Evaluation Input;
- the minimum Platform Health Evidence path reports its own blind spots and remains informative when the main pipeline, Backend or Query Runtime fails;
- ordinary operational Telemetry does not become Audit Evidence; sensitive query/export, sampling/drop policy and recovery actions submit structured audit records;
- raw Telemetry lifecycle remains with the Backend owning Contract, while COP metadata/reference/Evaluation Input retains separate lifecycle and reconciliation;
- Secret and credential material cannot enter ordinary Telemetry, metadata, error, checkpoint or queue payload;
- Resume, Replay, Reconcile and Never infer have distinct validation evidence, and restart/connectivity/queue drain/single query do not mean recovery success;
- all three Evolution Profiles only strengthen isolation, redundancy, capacity, recovery and governance;
- the document remains `draft` and introduces no RFC/ADR, fixed product, protocol, schema, topology, capacity, retention value, sampling value, RPO/RTO or numeric SLO.
...
if ($relativeLinks -ne 226) { throw "Expected 226 relative links, found $relativeLinks" }
git diff --check
if ($LASTEXITCODE -ne 0) { throw 'git diff --check failed' }
$changed = @(git diff --name-only)
if ($changed.Count -ne 1 -or $changed[0] -ne 'docs/infrastructure/observability-stack.md') { throw "Unexpected file scope: $($changed -join ', ')" }
'PASS: repository encoding, links and scope'
```

Expected output: `PASS: repository encoding, links and scope`.

- [ ] **Step 6: Commit the Observability Stack Architecture document**

```powershell
git add docs/infrastructure/observability-stack.md
git diff --cached --check
git commit -m "docs: define observability stack architecture"
```

Expected: one commit containing only `docs/infrastructure/observability-stack.md`.

- [ ] **Step 7: Verify the implementation commit**

```powershell
$files = @(git diff-tree --no-commit-id --name-only -r HEAD)
if ($files.Count -ne 1 -or $files[0] -ne 'docs/infrastructure/observability-stack.md') { throw "Unexpected committed files: $($files -join ', ')" }
if (git status --porcelain) { throw 'Worktree is not clean' }
'PASS: Observability Stack Architecture commit'
```

Expected output: `PASS: Observability Stack Architecture commit`.

### Plan Self-Review

- [ ] Confirm every approved requirement in `docs/superpowers/specs/2026-08-12-observability-stack-content-design.md` maps to Task 1 Step 2, Step 3 or Step 4.
- [ ] Confirm the plan contains no placeholder text, product choice, fixed protocol/schema/topology, numeric threshold, unsupported authority transfer or unapproved status promotion.
- [ ] Confirm target front matter, H2/H3 order, Mermaid relations, table headers, five capability roles, thirteen related IDs/references and repository link count are identical between Step 2 and the GREEN gates.
