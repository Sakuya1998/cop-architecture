...
**Goal:** 在保持 `COP-ARCH-004` 稳定 ID 与 `draft` 门禁的前提下，按交互语义定义同步 Query/Command、异步 Task/Event、Webhook 和 Provider/Connector Adapter 的所有权、交付、错误、隔离及兼容边界。

**Architecture:** 采用交互语义矩阵而不是按协议或产品分层。领域/能力 owner 拥有业务 Contract；API、Event/Webhook Delivery 和 Adapter 只负责安全、路由、投递、适配和错误归一化，不形成中央业务中枢。

**Tech Stack:** Markdown、YAML front matter、Mermaid `flowchart`、PowerShell 验证、Git。

---

### Task 1: Write and Verify COP Integration Architecture

**Files:**
- Modify: `docs/architecture/integration-architecture.md`
- Test: PowerShell front matter、章节、Mermaid、交互矩阵、语义、链接、UTF-8 和改动范围检查
...
$path = 'docs/architecture/integration-architecture.md'
...
Replace `docs/architecture/integration-architecture.md` with this complete content. Keep the four existing reference links exactly as shown.
...
$h3Expected = @('Integration Responsibility Model','Integration Paths Diagram','Interaction Semantics Matrix','Synchronous Contracts','Asynchronous Tasks and Events','Webhook Delivery','Provider and Connector Adapters','Error Isolation and Recovery','Versioning and Compatibility','Success Criteria')
...
$required = @(
  'flowchart LR','Sync API Boundary','Event and Webhook Delivery','Provider and Connector Adapter','Domain or Capability Owner',
  'accepted 不等于 completed','operation/task ID','at-least-once','不承诺 exactly-once','idempotency key',
  'duplicate、out-of-order、delay 和 replay','partition/aggregate scope','tenant/subscription 使用独立 endpoint',
  'controlled credential reference','不能回滚领域事实','validation、authentication、authorization、rate limit、timeout、dependency unavailable、conflict、partial 和 unknown',
  'tenant、subscription、consumer、Provider/Connector 和外部 dependency','双版本迁移窗口','forward-recovery',
  'COP-ARCH-002','COP-API-001','COP-API-002','COP-API-003','COP-DOM-004'
)
...
if ($tableRows.Count -ne 7) { throw "Expected 7 interaction table rows including header and separator, found $($tableRows.Count)" }
foreach ($row in $tableRows) {
  if (([regex]::Matches($row, '\|')).Count -ne 5) { throw "Table column mismatch: $row" }
}
...
if ([regex]::Matches($diagram, 'SUBSCRIBERS\s*--.*OWNER').Count -ne 0) { throw 'Diagram implies subscriber rollback or owner write path' }
...
'PASS: integration architecture structure and semantics'
```

Expected output: `PASS: integration architecture structure and semantics`.

- [ ] **Step 4: Verify the diagram and integration boundaries manually**

Inspect the rendered or source diagram and require all of the following:

- Domain/Capability Owner is the only business-semantic owner; no integration node owns domain writes or invariants.
- Sync API, Event/Webhook Delivery and Provider/Connector Adapter are distinct boundaries, not a linear service chain.
- Async arrows show owner-to-delivery-to-subscriber flow without a subscriber-to-owner rollback or write path.
- Provider/Connector uses an Adapter boundary and reaches the owner only through an owner Contract.
- The diagram does not imply exactly-once, global ordering, distributed transactions, shared writable storage or a central Integration Service.
- The interaction table and diagram use the same names and relationships.
...
if ($LASTEXITCODE -ne 0) { throw 'git diff --check failed' }
$changed = @(git diff --name-only)
if ($changed.Count -ne 1 -or $changed[0] -ne 'docs/architecture/integration-architecture.md') { throw "Unexpected file scope: $($changed -join ', ')" }
'PASS: repository links and scope'
```

Expected output: `PASS: repository links and scope`.

- [ ] **Step 6: Commit the integration architecture document**

```powershell
git add docs/architecture/integration-architecture.md
git commit -m "docs: define integration architecture"
```
...
if ($files.Count -ne 1 -or $files[0] -ne 'docs/architecture/integration-architecture.md') { throw "Unexpected committed files: $($files -join ', ')" }
if (git status --porcelain) { throw 'Worktree is not clean' }
'PASS: integration architecture commit'
```

Expected output: `PASS: integration architecture commit`.
