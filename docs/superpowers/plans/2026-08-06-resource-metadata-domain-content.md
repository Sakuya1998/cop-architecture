# COP Resource Metadata 领域内容实施计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 在保持 `COP-DOM-003` 稳定 ID 与 `draft` 门禁的前提下，将 Resource Metadata 初始化骨架补充为可评审领域文档。

**Architecture:** 采用 capability-aligned aggregates，分离 Resource Identity、Resource Observation、Type/Attribute Schema、Relationship 和 Resource Context Projection。Cloud Provider/Kubernetes 保留实际状态权威，Cloud Access 只发布 Observation；Resource Metadata 独占 COP 资源身份、规范化元数据、关系和生命周期，并通过显式 source、version、freshness、Tenant scope 与 fail-closed 语义向下游提供受治理上下文。

**Tech Stack:** Markdown、YAML front matter、Mermaid `flowchart`、PowerShell 文档校验、Git。

---

### Task 1: Write and Verify COP Resource Metadata Domain

**Files:**
- Modify: `docs/domains/resource-metadata-domain.md`
- Test: PowerShell front matter、章节顺序、Mermaid、聚合表、生命周期、身份/来源/关系/Observation 语义、链接、UTF-8、占位符和单文件范围检查
...
$path = 'docs/domains/resource-metadata-domain.md'
...
foreach ($value in $required) { if (-not $content.Contains($value)) { throw "Missing: $value" } }
```

Expected result: `Missing: version: 0.2.0`, proving the initialization skeleton does not satisfy the approved design.

- [ ] **Step 2: Replace the target document with the complete approved content**

Replace only `docs/domains/resource-metadata-domain.md` with the following content. Preserve the stable ID, `draft` status, existing owners/related/rfc/adr values, and all existing relative reference targets.
...
$h3Expected = @('Resource Metadata Responsibility Boundary','Identity, Tenant, Type and Attribute Model','Observation and Lifecycle Model','Relationship and Resource Context Model','Failure and Recovery','Validation Strategy','Success Criteria','Commands','Queries','Events','Audit')
...
foreach ($value in @(
  'Cloud Access 只发布 Resource Observation', '唯一 COP Resource Identity', 'Cloud Provider/Kubernetes 保留实际资源状态权威',
  '不可变内部 `resource_id`', 'provider', 'account/cluster', 'external_id', 'tombstone', 'Resource ID',
  '每个 Resource 只属于一个 Tenant', '跨 Tenant 访问默认拒绝', 'source', 'observed-at/version', 'freshness', '禁止 last-write-wins',
  'namespace/key', 'schema version', '敏感级别', '未知或不兼容属性隔离保留', '类型化、有向、带有效期和版本',
  '发现关系与治理关系分开建模', 'Resource Context', 'as_of', 'failure semantics', 'stale', 'partial', 'degraded',
  '幂等 upsert', 'duplicate', 'out-of-order', 'replay', 'reconciliation', 'at-least-once', 'expected version', 'idempotency key',
  'fail closed', 'Discovered', 'Active', 'Stale', 'Deleted', 'Resource Metadata Responsibility Boundary', 'Resource Context Projection'
...
foreach ($value in @('ACCESS --> OBSERVATION','OBSERVATION --> RESOURCE','TYPE --> RESOURCE','RESOURCE --> RELATION','RESOURCE --> CONTEXT','RELATION --> CONTEXT','CONTEXT --> OBS','CONTEXT --> ALERT','CONTEXT --> EXPERIENCE','IAM -.-> RESOURCE','IAM -.-> CONTEXT')) { if (-not $diagram.Contains($value)) { throw "Missing diagram relation: $value" } }
$rows = [regex]::Match($content, '(?ms)^\| Aggregate or capability \| Ownership \| Key references and boundaries \|\n(.*?)\n\n').Groups[1].Value -split "`n"
if ($rows.Count -ne 6) { throw "Expected separator and 5 aggregate rows, found $($rows.Count)" }
...
'PASS: Resource Metadata domain structure and semantics'
```

Expected output: `PASS: Resource Metadata domain structure and semantics`.
...
- Cloud Provider/Kubernetes owns actual state; Cloud Access only publishes Observation; Resource Metadata alone owns COP Resource Identity, normalized metadata and relationships.
- Internal Resource ID is immutable; external locator key is provider/account-or-cluster/type/external ID; Deleted tombstones never reuse IDs.
- A Resource belongs to exactly one Tenant in MVP; cross-Tenant reads, relationships and Projections fail closed.
- Provider facts are read-only projections; COP managed attributes are not overwritten by discovery; source/freshness/version merge is deterministic.
- Attributes are typed and namespaced; unknown or incompatible schema is quarantined without corrupting committed facts.
- Relationships are typed, directed, source-aware, versioned and time-bounded; discovered and governed relations remain distinguishable.
- Observation ingestion is idempotent and replayable; duplicate, out-of-order, schema-invalid and unresolved-target inputs are isolated.
- `Stale` can return to `Active`; confirmed `Deleted` remains a tombstone and re-discovery creates a new Resource ID.
- Resource Context declares owner/source/as_of/freshness/failure semantics; stale/partial/degraded/unavailable is never presented as a complete success.
- Events are at-least-once and minimal; no shared storage, last-write-wins, exactly-once, global ordering or distributed transaction is implied.
...
if ($LASTEXITCODE -ne 0) { throw 'git diff --check failed' }
$changed = @(git diff --name-only)
if ($changed.Count -ne 1 -or $changed[0] -ne 'docs/domains/resource-metadata-domain.md') { throw "Unexpected file scope: $($changed -join ', ')" }
'PASS: repository links and scope'
```

Expected output: `PASS: repository links and scope`.

- [ ] **Step 6: Commit the Resource Metadata domain document**

```powershell
git add docs/domains/resource-metadata-domain.md
git commit -m "docs: define resource metadata domain content"
```
...
if ($files.Count -ne 1 -or $files[0] -ne 'docs/domains/resource-metadata-domain.md') { throw "Unexpected committed files: $($files -join ', ')" }
if (git status --porcelain) { throw 'Worktree is not clean' }
'PASS: Resource Metadata domain commit'
```

### Plan Self-Review

- [ ] Compare this plan with all 15 acceptance conditions in `docs/superpowers/specs/2026-08-06-resource-metadata-domain-content-design.md`; conditions 1-12 are covered by target sections, exact GREEN assertions and manual review, condition 13 by eight link and placeholder checks, condition 14 by byte-level UTF-8/Mermaid/table checks, and condition 15 by diff and single-file commit checks.
- [ ] Confirm the plan preserves `COP-DOM-003` metadata and four existing references, changes no index or related domain document, and never promotes status from `draft`.
- [ ] Confirm identity, lifecycle, source merge, Tenant isolation, Attribute schema, relationship direction, Observation idempotency, Resource Context freshness and fail-closed semantics are represented in target content and checks.
- [ ] Confirm there are no literal filler placeholders, invented products, deployment decisions, or API/database implementation details in the target content.
...
```powershell
git add docs/superpowers/plans/2026-08-06-resource-metadata-domain-content.md
git commit -m "docs: plan resource metadata domain content"
```
